...
from app.schemas import RegisterSchema,AuthResponse,LoginSchema,TokenSchema
from app.services.auth import AuthService

auth_router=APIRouter(prefix="/api/auth",tags=["Authentication"])

@auth_router.post("/user/login",response_model=AuthResponse,status_code=status.HTTP_200_OK)
async def login_user(data:LoginSchema,db:AsyncSession=Depends(get_db)):
  return await AuthService.login_user(data,db)

@auth_router.post("/user/register",response_model=AuthResponse,status_code=status.HTTP_201_CREATED)
